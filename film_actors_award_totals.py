...
from oscar_scrape import CAST_CSV_FILE, CAST_FIELDNAMES
...
def main() -> None:
    ...
    feature_cols, prefixes = load_matrix_prefixes(args.matrix)
    nfeat = len(feature_cols)
    out_fieldnames = list(CAST_FIELDNAMES) + feature_cols

    written = 0

    with open(args.film_actors, newline="", encoding="utf-8") as inf, open(
        args.output, "w", newline="", encoding="utf-8"
    ) as outf:
        reader = csv.DictReader(inf)
        missing = [c for c in CAST_FIELDNAMES if c not in (reader.fieldnames or [])]
        if missing:
            raise SystemExit(f"--film-actors missing columns: {missing}")
    ...
    print(f"Wrote {written} rows to {args.output} ({nfeat} feature columns).")
