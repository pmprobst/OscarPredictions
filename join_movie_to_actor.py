...
from oscar_scrape import CSV_FILE
...
def load_sums_aggregates(
    ...
    with open(sums_path, newline="", encoding="utf-8") as f:
        ...
        missing = need - {c.strip() for c in fn}
        if missing:
            raise SystemExit(f"{sums_path} missing columns: {sorted(missing)}")
...
def main() -> None:
    ...
    feature_cols, aggregates = load_sums_aggregates(args.film_actors_sums)
    nfeat = len(feature_cols)
    zeros = [0] * nfeat

    with open(args.movies, newline="", encoding="utf-8") as mf:
        m_reader = csv.DictReader(mf)
        if not m_reader.fieldnames:
            raise SystemExit(f"Empty or invalid movies CSV: {args.movies}")
        movie_cols = list(m_reader.fieldnames)
        miss_m = MOVIES_REQUIRED - {c.strip() for c in movie_cols}
        if miss_m:
            raise SystemExit(f"{args.movies} missing columns: {sorted(miss_m)}")

        include_count = not args.no_cast_count
